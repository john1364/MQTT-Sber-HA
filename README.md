...
- Поддерживает **реле** (выключатели, лампы, кнопки, сценарии), **розетки с энергомониторингом** (мощность, ток, напряжение), **кондиционеры** (с управлением скоростью вентилятора и направлением потока), **увлажнители воздуха**, **чайники** (с уровнем воды), **пылесосы**, **краны**, **лампы** (яркость, цвет, цветовая температура), **рулонные шторы / жалюзи**, **бризеры / вентиляторы** (fan + climate + switch), **термоголовки радиатора**, **телевизоры** (громкость, mute, каналы, источник), **домофоны** (открытие двери + датчик вызова), **датчики температуры/влажности**, **датчики открытия двери/окна**, **датчики качества воздуха** (CO₂, PM2.5, TVOC), **датчики протечки**, **датчики дыма** и **сценарные кнопки**
